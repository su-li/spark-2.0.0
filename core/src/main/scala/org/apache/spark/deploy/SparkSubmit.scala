...
object SparkSubmit {
  ...
  private[deploy] def prepareSubmitEnvironment(args: SparkSubmitArguments)
    ...
    //sysProps 中放入spark的运行系统参数  master  deploymode  memeory 等等
    for (opt <- options) {
      if (opt.value != null &&
    ...
    //如果是standalone模式 且部署模式为cluster  那么默认用RestSubmissionClient
    if (args.isStandaloneCluster) {
      if (args.useRest) {  //useRest默认为true
