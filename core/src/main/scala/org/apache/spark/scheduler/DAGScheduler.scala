...
class DAGScheduler(
  ...
  //new 一个DAGSchedulerEventProcessLoop  点进去这个类-->
  ...
  private def getShuffleMapStage(
    ...
    shuffleToMapStage.get(shuffleDep.shuffleId) match {
      ...
      case None =>
        // We are going to register ancestor(被继承者) shuffle dependencies
    ...
    def visit(r: RDD[_]) {
      if (!visited(r)) {
        ...
        for (dep <- r.dependencies) {
          dep match {
              //若和此父rdd的关系是宽依赖,则拿父rdd的依赖
            case shufDep: ShuffleDependency[_, _, _] =>
              parents += getShuffleMapStage(shufDep, firstJobId)
            case _ =>
              //若和此父rdd的关系是窄依赖,则循环往前找直到找到宽依赖
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    try {
      ...
      //通过newResultStage方法得到所有的stage集合
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    val tasks: Seq[Task[_]] = try {
      stage match {
        ...
        //根据分区数把一个stage切分为多个task 放入Seq序列
        case stage: ResultStage =>
...
//此类在创建时,父类的构造方法会被调用   传入字符串dag-scheduler-event-loop  点进去父类 -->
